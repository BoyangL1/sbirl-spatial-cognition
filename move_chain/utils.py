...
def loadTrajChain(traj_file,num_trajs=None):
    ...
    for tc in traj_chains:
        for t in range(len(tc.travel_chain)-1):
            ...
            a_n_a = onp.zeros((2,1))
            a_n_a[0] = tc.travel_chain[t+1]
            try:
                a_n_a[1] = tc.travel_chain[t+2]
            except IndexError:
                a_n_a[1] = 0 # end action
    ...
    a_dim = len(np.unique(action_next_action))
...
if __name__ == "__main__":
    ...
    inputs, targets, a_dim, s_dim =loadTrajChain(path)