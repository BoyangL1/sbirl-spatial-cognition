...
class avril:
    ...
    def train(self, iters: int = 1000, model_save_path: str='./model/params_special_graph.pickle', batch_size: int = 64, l_rate: float = 1e-4):
        """
...
def qvalue(model,pathGraph):
    G = nx.read_graphml(pathGraph)
    data_list = []

    for id in list(G.nodes()):
        state = np.array(json.loads(G.nodes[str(id)]['feature']))
        q_values = model.q_network.apply(
            model.q_params,
            model.key,
            state,
            model.a_dim,
        ...
        row_data = [id] + list(q_values)
        data_list.append(row_data)

    columns = ['fnid'] + [str(i) for i in range(35)]
    df = pd.DataFrame(data_list, columns=columns)
    df.to_csv('./data/communityGraph/graph_Qvalue.csv',index=False)

if __name__ == "__main__":
    path = f"./data/communityGraph/all_walks.pkl"
    pathGraph = f'./data/communityGraph/communityGraph.graphml'
    inputs,targets,a_dim, s_dim=loadTraj(path,pathGraph,None)
